...
def getData(name, interferrometer, duration):
    ...
    ax.plot(data.times.value, data.value)
    ax.set_title(f"{name} {interferrometer} Waveform")
    ax.set_xlabel('Time [s]')
    ax.set_ylabel('Strain')
    plt.savefig(f"{name}_{interferrometer}_waveform.png", dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax.loglog(tempInterferrometer.power_spectral_density.frequency_array[mask_psd],
        tempInterferrometer.power_spectral_density.asd_array[mask_psd], label = 'Raw PSD')
    ...
    ax.loglog(tempInterferrometer.strain_data.frequency_array[mask_strain],
        np.abs(tempInterferrometer.strain_data.frequency_domain_strain[mask_strain]), label = 'Masked PSD')


    ax.set_xlim(10,1000)
    ax.set_xlabel("Frequency [Hz]")
    ax.set_ylabel(r"Strain [strain/$\sqrt{Hz}$]")
    ax.set_title(name + " " + interferrometer + " PSD")
