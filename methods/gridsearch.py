...
for det in detectors:
    ...
    try:
        strain = TimeSeries.fetch_open_data(
            det, time - 32, time + 32, sample_rate=4096, cache=True)
        ...
        print("Successfully fetched data from " + det + "\n")

        psds[det] = psd
        data[det] = ts
    except:
        print("Failed to fetch data from " + det + "...\n")
        detectors.remove(det)
    

# ── 2. Set up the finer 2D grid of trial (m1, m2) templates ────────────
m1_grid = np.linspace(15, 60, 60)
m2_grid = np.linspace(20, 60, 60)

snr_grid = np.full((len(m1_grid), len(m2_grid)), np.nan)

print("Running 2D matched-filter grid search "
...
plt.colorbar(mesh, label="Network matched-filter SNR")
...
#ax.scatter(
#    7.3, 5.14, color="white", marker="x", s=120,
...
plt.tight_layout()
plt.savefig(name + " grid search.png", dpi=500)
plt.show()